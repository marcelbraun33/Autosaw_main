...
// Define button IDs for all interface elements
#ifndef WINBUTTON_CAPTURE_CUT_START_F6
#define WINBUTTON_CAPTURE_CUT_START_F6 30    // "Capture position as Start"
#endif

#ifndef WINBUTTON_CAPTURE_CUT_END_F6
#define WINBUTTON_CAPTURE_CUT_END_F6 27      // "Capture position as Cut End" 
#endif

#ifndef WINBUTTON_CAPTURE_RETRACT_F6
#define WINBUTTON_CAPTURE_RETRACT_F6 39      // "Capture position as Safe Retract"
#endif

#ifndef WINBUTTON_ZERO_TO_START_F6
#define WINBUTTON_ZERO_TO_START_F6 34        // "Zero to Start Position"
#endif

#ifndef WINBUTTON_SET_LENGTH_WITH_MPG_F6
#define WINBUTTON_SET_LENGTH_WITH_MPG_F6 29  // "Set with MPG" for cut length
#endif

#ifndef WINBUTTON_SET_RETRACT_WITH_MPG_F6 
#define WINBUTTON_SET_RETRACT_WITH_MPG_F6 40 // "Set with MPG" for retract distance
#endif

#ifndef WINBUTTON_ACTIVATE_JOG_Y
#define WINBUTTON_ACTIVATE_JOG_Y 28          // "Enable Jog" button
#endif

#ifndef WINBUTTON_GO_TO_RETRACT_F6
#define WINBUTTON_GO_TO_RETRACT_F6 41        // "Go to Safe Retract" button 
#endif

// LED digit display IDs
#ifndef LEDDIGITS_STOCK_END_Y
#define LEDDIGITS_STOCK_END_Y 19        // Cut Start point display
#endif

#ifndef LEDDIGITS_TABLE_POSITION_Y
#define LEDDIGITS_TABLE_POSITION_Y 20   // Current table position display
#endif

#ifndef LEDDIGITS_CUT_LENGTH_Y
#define LEDDIGITS_CUT_LENGTH_Y 21       // Cut length display
#endif

#ifndef LEDDIGITS_CUT_STOP_Y
#define LEDDIGITS_CUT_STOP_Y 22         // Cut stop point display
#endif

#ifndef LEDDIGITS_RETRACT_DISTANCE_Y
#define LEDDIGITS_RETRACT_DISTANCE_Y 25 // Retract distance display
#endif
...
void JogYScreen::onShow() {
    ...
    // Reset button states
    showButtonSafe(WINBUTTON_SET_LENGTH_WITH_MPG_F6, 0);
...
void JogYScreen::handleEvent(const genieFrame& e) {
    ...
    case WINBUTTON_ZERO_TO_START_F6:
        zeroToStartPosition();
        break;

    case WINBUTTON_GO_TO_RETRACT_F6:
        moveToRetractPosition();
        break;

    case WINBUTTON_SET_LENGTH_WITH_MPG_F6:
        setLengthWithMPG();
        break;
...
void JogYScreen::captureCutStart() {
    ...
    // Since we're at the start position, illuminate LED1
    genie.WriteObject(GENIE_OBJ_LED, LED_AT_START_POSITION_Y, 1);
...
void JogYScreen::zeroToStartPosition() {
    ...
    // Illuminate LED1 to indicate at start position
    genie.WriteObject(GENIE_OBJ_LED, LED_AT_START_POSITION_Y, 1);
...
void JogYScreen::moveToRetractPosition() {
    ...
    // Visual feedback
    showButtonSafe(WINBUTTON_GO_TO_RETRACT_F6, 1);
    delay(200);
    showButtonSafe(WINBUTTON_GO_TO_RETRACT_F6, 0);
}
...
void JogYScreen::setLengthWithMPG() {
    ...
    if (_mpgSetLengthMode) {
        ...
        if (mpg.isEnabled()) {
            ...
            showButtonSafe(WINBUTTON_ACTIVATE_JOG_Y, 0);
        }
        ...
        showButtonSafe(WINBUTTON_SET_LENGTH_WITH_MPG_F6, 1);

        // Bind to edit field with fixed 5 thousandths increment
        ui.bindField(WINBUTTON_SET_LENGTH_WITH_MPG_F6, LEDDIGITS_CUT_LENGTH_Y,
            &_tempLength, 0.0f, 100.0f, MPG_FIXED_INCREMENT, 3);
    }
    else {
        // Exit MPG length edit mode
        if (ui.isFieldActive(WINBUTTON_SET_LENGTH_WITH_MPG_F6)) {
...
void JogYScreen::setRetractWithMPG() {
    ...
    if (_mpgSetRetractMode) {
        ...
        // Show button as active - using genie.WriteObject directly for immediate effect
        ...
        ClearCore::ConnectorUsb.SendLine(LEDDIGITS_RETRACT_DISTANCE_Y);

        ui.bindField(WINBUTTON_SET_RETRACT_WITH_MPG_F6, LEDDIGITS_RETRACT_DISTANCE_Y,
        ...
        // Show button as inactive
        genie.WriteObject(GENIE_OBJ_WINBUTTON, WINBUTTON_SET_RETRACT_WITH_MPG_F6, 0);
    }
}
...
void JogYScreen::update() {
    ...
    // Check if position is within 0.002 inches of the cut start point
    // and illuminate LED1 accordingly
    float distanceToStart = fabs(pos - _cutStartPoint);
    if (distanceToStart <= 0.002f) {
        // Table is at or very close to start position - illuminate LED1
        genie.WriteObject(GENIE_OBJ_LED, LED_AT_START_POSITION_Y, 1);
    }
    else {
        // Table is not at start position - turn off LED1
        genie.WriteObject(GENIE_OBJ_LED, LED_AT_START_POSITION_Y, 0);
    }
    ...
    if (now - lastCheckTime > 500) {  // Check every 500ms
        ...
        if (_mpgSetLengthMode) {
            genie.WriteObject(GENIE_OBJ_WINBUTTON, WINBUTTON_SET_LENGTH_WITH_MPG_F6, 1);
        }
